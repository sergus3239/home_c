...
void Sort_Last_Digit(int n, int arr[])
{
	//int temp;
	for (int i = 0; i < n - 1; i++)
	{
...
*/
void Sort_Incr(int n, int arr[], int start, int end)
{
...
void Sort_Decr(int n, int arr[], int start, int end)
{
	for (int i=start; i < end; i++)
	{
		for (int j = start; j < end - (i-start); j++)
...
int main()
{
	...
	//int k=4;
	Input_Arr(n, arr);
	Sort_Incr(n, arr, n-n, n/2-1);
	Sort_Decr(n, arr, n/2, n-1);
	//Sort_Last_Digit(n, arr);
